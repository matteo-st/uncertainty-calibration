...
\begin{enumerate}
  ...
  We conduct extensive experiments on standard vision and natural-language benchmarks, showing that one can obtain uncertainty scores that remain strongly discriminative for error detection while becoming interpretable as calibrated failure risk.
\end{enumerate}
...
In the experiments, we apply the proposed post-hoc 
We conduct extensive experiments on standard vision and natural-language benchmarks, in order to show that one can obtain uncertainty scores uncertainty scores that remain strongly discriminative for error detection while being interpretable with certified guarantees. Concretely, for different models for text classification and image classification, we implement existing uncertainty scores and apply the post-hoc calibration procedure discussed in Section~\ref{sec:calib_algo}. 
\subsection{Metrics}

To measure the discriminative performance of an uncertainty score for misclassification detection (MisD), we report the area under the ROC curve (AUROC~$\uparrow$). To measure calibration, we report the Expected Calibration Error (ECE) and the Maximum Calibration Error (MCE). For an uncertainty score $u$ taking discrete values $u(\mathcal{X})=\{t_1,\dots,t_T\}$, and for an empirical distribution $\hat P$, we define
\begin{align*}
    \mathrm{ECE}(u)
...
\]
\textbf{Remark:} For a score $u$ taking continuous values, we first discretize it using equal-mass binning. Note, however, that calibration errors computed after discretization can underestimate the true miscalibration of the underlying continuous score \citep{kumar_verified_2020}.
\subsection{Text Classification}


\textbf{Datasets} We consider two datasets from the GLUE benchmark \citep{wang-etal-2018-glue} that were previously use by \citep{vazhentsev_uncertainty_2022} for error detection: Microsoft Research Paraphrase Corpus (MRPC) \citep{dolan2005automatically} and Stanford Sentiment Treebank (SST-2) \citep{socher-etal-2013-recursive}. The data statistics are presented in Table~\ref{tab:dataset_stats} in Appendix~\ref{app:expe}.

\textbf{Models.} We use two Transformers: the pre-trained ELECTRA model \citep{clark2020electrapretrainingtextencoders} with 110 million parameters and DeBERTa \citep{he2021debertadecodingenhancedbertdisentangled} with 138 million parameters. To adapt the models to each dataset, we add a classification head and fine-tune all the model weights on the training sets. We use the same hyperparameter values given by \citep{vazhentsev_uncertainty_2022}. 

\textbf{Uncertainty scores.} We compare the baseline softmax probability (SP) score \citep{hendrycks2018baselinedetectingmisclassifiedoutofdistribution} with the Mahalanobis-distance (MD) score of \citet{vazhentsev_uncertainty_2022}. We present those methods in Appendix~\ref{app:uncertainty_scores}.

\subsection{Image Classification}

\textbf{Datasets \& Models.}
We consider \textit{CIFAR-10}~\cite{Krizhevsky2009LearningML}, \textit{CIFAR-100}~\cite{Krizhevsky2009LearningML}, and \textit{ImageNet-1K}~\cite{imagenet}. The numbers of classes are 10, 100, and 1000, respectively. For CIFAR-10/100 we use the standard test splits (each of size \(10\mathrm{k}\)); for ImageNet-1K we use the standard validation split (50\(\mathrm{k}\) images).

\textbf{Models.}
We evaluate four architectures: DenseNet-121, ResNet-34, ViT-B/16, and ViT-Tiny/16.
To reduce compute and energy usage and to enhance reproducibility, we rely on publicly
available pretrained weights: for DenseNet-121 and ResNet-34 we use the models released
by \citet{dadalto2024datadrivenmeasurerelativeuncertainty}; for ViT-B/16 and ViT-Tiny/16
we use the \texttt{timm} releases (\texttt{vit\_base\_patch16\_224} and
\texttt{vit\_tiny\_patch16\_224}, respectively)~\citep{rw2019timm}.

\textbf{Uncertainty scores.}
We consider three widely used misclassification detection (MisD) scores: ODIN~\citep{liang2020enhancingreliabilityoutofdistributionimage} (a modification of the softmax-probability (SP) baseline), Doctor~\citep{granese2021doctorsimplemethoddetecting}, and the margin score~\citep{settles2009active}. We describe these methods in Appendix~\ref{app:uncertainty_scores}.

\subsection{Results.}

\input{tables/nlp_results}

\section{Summary and Future works}
We studied the constructionn of uncertainty score that are both discriminative for misclassifcation detection and which enjoy calibration guarantees. 


Future work includes tightening the certificates via sharper one-sided binomial bounds (e.g., Clopper--Pearson, empirical-Bernstein-type intervals, Sterne-type exact intervals, Blaker’s). This could include modifying the uniform-mass quantization by rather adapting the number of samples in each bin depending on the apriori difficulty of Bernoulli estimation. This apriori would depend both on the performance of the classifier, and how the used uncertainty scores is assumed to approximate the error probability. 
...
\subsection{Dataset Statistics and Hyperparameter Values}
...
$g(x)=(g_1(x),\dots,g_K(x))$ on the probability simplex. The associated classifier is
\[
f(x) \in \arg\max_{k\in[K]} g_k(x).
\]

\paragraph{Softmax probability (SP).}
The softmax-probability (maximum softmax probability) score is defined as
\[
u_{\mathrm{SP}}(x) := 1 - \max_{k\in[K]} g_k(x) = 1 - g_{f(x)}(x).
\]
When an \emph{uncertainty} score is required (higher means more uncertain), we use the monotone transform
\[
u_{\mathrm{SP\text{-}unc}}(x) := 1-u_{\mathrm{SP}}(x).
\]

\paragraph{Mahalanobis distance (MD).}
Following \citet{vazhentsev_uncertainty_2022}, we quantify uncertainty by measuring the Mahalanobis distance between a test instance and the closest class-conditional Gaussian distribution. Let $h_i\in\mathbb R^d$ be the hidden representation of instance $i$, and let $\mathcal C=\{1,\dots,K\}$ denote the set of classes. Define the MD score
\begin{equation}
u_{\mathrm{MD}}(h_i)
~:=~
\min_{c\in\mathcal C}
...
\label{eq:md_score}
\end{equation}
where $h_i$ is a hidden representation of the $i$-th instance, $\mu_c$ is the centroid of class $c$, and $\Sigma$ is a covariance matrix for hidden representations of training instances. Larger values of $u_{\mathrm{MD}}$ indicate that the representation is far from all class centroids, hence higher uncertainty.
