...
To limit computational cost, we subsample SST-2 and AG News to 10\% of their original training sets (from 67{,}349 to 6{,}735 and from 120{,}000 to 12{,}000 samples, respectively); MRPC and CoLA are used in full.
For each dataset, we hold out a calibration set of $n=1{,}000$ samples from the training data, reserved exclusively for fitting the calibrator and never used during training or hyperparameter selection.
For MRPC and CoLA, the test set is the GLUE validation split~\citep{wang-etal-2018-glue} (since GLUE test labels are not publicly available).
For SST-2, we carve a test set of 7{,}600 samples from the unused training pool (after the train and calibration splits) to obtain a larger evaluation set; for AG News, we use its standard test split.
Full dataset statistics are reported in Table~\ref{tab:dataset_stats} (Appendix~\ref{app:expe}).
...
Hyperparameters (learning rate, batch size, number of epochs, weight decay) are selected via Bayesian optimization (Optuna TPE, 20 trials) on an internal 80/20 train/validation split of the training data; the calibration set is never used for selection.
After selection, each model is re-trained on the full training set (excluding the calibration set) with the chosen hyperparameters.
The selected hyperparameters and search ranges are reported in Appendix~\ref{app:expe}.

\textbf{Uncertainty scores.}
We focus on two representative uncertainty scores from complementary families:
the \emph{softmax probability} (SP) score $u_{\mathrm{SP}}(\mathbf{x}) = 1 - \max_k g_k(\mathbf{x})$~\citep{hendrycks2018baselinedetectingmisclassifiedoutofdistribution}, the most widely used baseline, which is bounded in $[0,1]$ and directly interpretable as an error probability;
and the \emph{Mahalanobis distance} (MD)~\citep{vazhentsev_uncertainty_2022}, a density-based score computed from hidden representations, which is unbounded and captures information from the feature space rather than the output layer.
All scores are formally defined in Appendix~\ref{app:expe}, where we additionally evaluate three further scores (predictive entropy, Doctor~\citep{granese2021doctorsimplemethoddetecting}, and energy~\citep{liu2021energybasedoutofdistributiondetection}); none outperform SP in discrimination, and for binary tasks ($K=2$) the softmax-based scores are monotone transforms of SP yielding identical ROCAUC.
...
Table~\ref{tab:main_results} reports ROCAUC before and after uniform-mass (UM) calibration for each (dataset, model) pair.
The average ROCAUC change after calibration is $-0.017$ for SP and $-0.021$ for MD.
...
\paragraph{Low calibration error after UM calibration.}
MCE after UM calibration is consistently low: $0.140$ on average for SP and $0.155$ for MD (Table~\ref{tab:main_results}).
For SP---the only score directly interpretable as an error probability before calibration---raw MCE averages $0.347$, showing that UM calibration substantially reduces miscalibration.
Raw MD scores are unbounded and not directly interpretable as probabilities; UM calibration maps them to the $[0,1]$ interval with empirical error rates.

\paragraph{Comparison with parametric and nonparametric baselines.}
Figure~\ref{fig:ncal_ablation} shows how MCE and ROCAUC vary with the calibration set size $n_{\mathrm{cal}}$ for three methods: uniform-mass (UM), Platt scaling~\citep{plat_scaling} fitted as $\sigma(a z + b)$ on standardized scores, and isotonic regression~\citep{isotonic}.
We use the Mahalanobis distance score (the strongest discriminator) with DeBERTa-v3-base (the best-performing model) and vary $n_{\mathrm{cal}} \in \{50, 100, 200, 500, 1000, 2000, 5000, 10000\}$ by sampling from unused training data of SST-2 and AG News (which were subsampled to 10\%), with 20 random draws per value.
Results are shown separately for each dataset, aggregating over 3 seeds $\times$ 20 draws = 60 points per $n_{\mathrm{cal}}$ value.
The full per-model breakdown is in Appendix~\ref{app:ncal_ablation}.

UM's empirical MCE remains below the theoretical bound~$\varepsilon(n_{\mathrm{cal}})$ for $n_{\mathrm{cal}} \leq 5000$ on both datasets, confirming the finite-sample guarantee of Theorem~\ref{thm:unif-mass_calibration}.
At $n_{\mathrm{cal}} = 10{,}000$ the mean empirical MCE slightly exceeds~$\varepsilon$; this is expected since~$\varepsilon$ bounds the population MCE, while the empirical estimate on finite test sets becomes noisier as the number of bins~$B$ grows.
Platt scaling yields the highest MCE, reaching $0.28$ at $n_{\mathrm{cal}} = 10{,}000$, reflecting the rigidity of its parametric sigmoid form.
Isotonic regression achieves the lowest MCE across most calibration set sizes but lacks theoretical guarantees.
All three methods show degraded ROCAUC at small~$n_{\mathrm{cal}}$ due to estimation noise, but converge toward the raw score's discrimination for $n_{\mathrm{cal}} \geq 1000$.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/ncal_ablation.pdf}
\caption{Effect of calibration set size $n_{\mathrm{cal}}$ on maximum calibration error (MCE, left) and discrimination (ROCAUC, right) for the Mahalanobis distance score with DeBERTa-v3-base, shown separately for SST-2 (top) and AG News (bottom). Each point aggregates 60 experiments (3 seeds $\times$ 20 draws). Shaded bands show $\pm 1$ standard deviation. The dashed black curve is the theoretical bound $\varepsilon(n_{\mathrm{cal}})$ from Theorem~\ref{thm:unif-mass_calibration}. UM stays below the bound for $n_{\mathrm{cal}} \leq 5000$; Platt scaling has consistently higher MCE due to its rigid parametric form. Full per-model results in Figure~\ref{fig:ncal_ablation_full}.}
\label{fig:ncal_ablation}
\end{figure}
...
On the empirical side, experiments on four text classification benchmarks with three models and five uncertainty scores confirm that uniform-mass calibration provides low maximum calibration error (MCE $\approx 0.06$--$0.29$) while preserving discrimination (average ROCAUC drop $<0.02$).
...
\subsection{Calibration Sample Size Ablation --- Full Results}
\label{app:ncal_ablation}

Figures~\ref{fig:ncal_ablation_full} and~\ref{fig:ncal_ablation_sp} extend Figure~\ref{fig:ncal_ablation} to all three encoder models (ELECTRA, BERT, DeBERTa-v3-base), showing MCE and ROCAUC as a function of $n_{\mathrm{cal}}$ for each (model, dataset) combination.
Figure~\ref{fig:ncal_ablation_full} uses the Mahalanobis distance score; Figure~\ref{fig:ncal_ablation_sp} uses the softmax probability score.

The trends observed for DeBERTa in the main text hold across models: UM stays below the theoretical bound for moderate $n_{\mathrm{cal}}$, Platt scaling has rigidly high MCE, and all methods converge in ROCAUC for $n_{\mathrm{cal}} \geq 1000$.
Comparing the two scores confirms that Platt scaling's high MCE is not specific to unbounded scores: Platt MCE is similarly high for SP ($0.255$ at $n_{\mathrm{cal}}=10{,}000$) and MD ($0.220$), indicating that the sigmoid form is a poor model for the calibration map regardless of the score family.
This rules out the hypothesis that Platt scaling fails only on non-sigmoidal, unbounded scores; the rigidity of the two-parameter sigmoid is the fundamental limitation.

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/ncal_ablation_full.pdf}
\caption{Calibration sample size ablation --- Mahalanobis distance score. Each row is one (model, dataset) pair; columns show MCE (left) and ROCAUC (right). Each point aggregates 60 experiments (3 seeds $\times$ 20 draws). Shaded bands: $\pm 1$ std. Dashed black: theoretical $\varepsilon(n_{\mathrm{cal}})$.}
\label{fig:ncal_ablation_full}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/ncal_ablation_sp.pdf}
\caption{Calibration sample size ablation --- softmax probability score. Same layout as Figure~\ref{fig:ncal_ablation_full}. Platt scaling's MCE remains high (comparable to or higher than for the Mahalanobis score), confirming that the sigmoid model is poorly suited to the calibration map regardless of the score family.}
\label{fig:ncal_ablation_sp}
\end{figure}

Figures~\ref{fig:ncal_ablation_ece_md} and~\ref{fig:ncal_ablation_ece_sp} show the expected calibration error (ECE) as a function of $n_{\mathrm{cal}}$ for the same experimental setup.
Unlike MCE, ECE decreases monotonically with $n_{\mathrm{cal}}$ for all three methods, since it averages calibration errors across bins rather than taking the maximum.
Isotonic regression achieves the lowest ECE, followed by UM and Platt scaling.
The ordering is consistent across both scores, reinforcing the conclusion that Platt scaling's parametric rigidity affects both average and worst-case calibration.

\begin{figure}[h]
\centering
\includegraphics[width=0.65\linewidth]{figures/ncal_ablation_ece_md.pdf}
\caption{ECE vs $n_{\mathrm{cal}}$ --- Mahalanobis distance score. Each row is one (model, dataset) pair. Each point aggregates 60 experiments (3 seeds $\times$ 20 draws). Shaded bands: $\pm 1$ std.}
\label{fig:ncal_ablation_ece_md}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.65\linewidth]{figures/ncal_ablation_ece_sp.pdf}
\caption{ECE vs $n_{\mathrm{cal}}$ --- softmax probability score. Same layout as Figure~\ref{fig:ncal_ablation_ece_md}.}
\label{fig:ncal_ablation_ece_sp}
\end{figure}

\subsection{Hardware and Computational Resources}

All fine-tuning experiments were conducted on a server equipped with 2$\times$ NVIDIA A100 80\,GB GPUs.
Each of the 36 fine-tuning runs (12 model--dataset pairs $\times$ 3 seeds) takes between 5 and 30 minutes depending on dataset size and number of epochs.
Hyperparameter search (20 Optuna trials per pair, 12 pairs) was run in parallel across both GPUs.
Post-hoc calibration and metric computation are CPU-only and complete in seconds per experiment.
