...
For MRPC, SST-2, and CoLA, the test set is the GLUE validation split~\citep{wang-etal-2018-glue} (since GLUE test labels are not publicly available); for AG News, we use its standard test split.
...
Comparing the two scores provides insight into Platt scaling's failure mode: because Platt scaling fits a sigmoid $\sigma(az+b)$, it is well-suited to SP scores (which are bounded in $[0,1]$ and roughly sigmoidal), but poorly suited to MD scores (which are unbounded and have a non-sigmoidal calibration map).
Indeed, Platt scaling's MCE is substantially lower for SP than for MD across all models and datasets, confirming that its high MCE on MD is due to model misspecification rather than insufficient calibration data.
...
\caption{Calibration sample size ablation --- softmax probability score. Same layout as Figure~\ref{fig:ncal_ablation_full}. Platt scaling's MCE is substantially lower than for the Mahalanobis score (Figure~\ref{fig:ncal_ablation_full}), confirming that its poor calibration on MD is due to sigmoid model misspecification.}
