...
The average ROCAUC change after calibration is $-0.014$ for SP and $-0.020$ for MD.
...
MCE after UM calibration is consistently low: $0.127$ on average for SP and $0.144$ for MD (Table~\ref{tab:main_results}).
For SP---the only score directly interpretable as an error probability before calibration---raw MCE averages $0.367$, showing that UM calibration substantially reduces miscalibration.
...
Platt scaling yields the highest MCE, reaching $0.24$ at $n_{\mathrm{cal}} = 10{,}000$, reflecting the rigidity of its parametric sigmoid form.
...
On the empirical side, experiments on four text classification benchmarks with three models and five uncertainty scores confirm that uniform-mass calibration provides low maximum calibration error (MCE $\approx 0.03$--$0.32$) while preserving discrimination (average ROCAUC drop $<0.02$).
